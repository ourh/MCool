...
from CoolProp.CoolProp import Props #,Tsat             #refrigerant properties
...
class SecondaryCycleClass():
    ...
    def Calculate(self,DT_evap,DT_cond,Tin_IHX):
        ...
        self.Tbubble_evap=Props('T','P',psat_evap,'Q',0,self.Ref) #Tsat(self.Ref,psat_evap,0,0)
        ...
        hL=Props('H','T',self.Tbubble_evap,'Q',0.0,self.Ref)*1000
        hV=Props('H','T',self.Tdew_evap,'Q',1.0,self.Ref)*1000
        xin_r=(self.Condenser.hout_r-hL)/(hV-hL)
        params={
            'mdot_h': self.Pump.mdot_g,
            'hin_h': Props('H','T',Tin_IHX,'P',300,Cycle.SecLoopFluid)*1000,
